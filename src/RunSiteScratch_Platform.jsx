...
function formatIntake(fd, vertical, tier) {
  ...
  if (vertical === "cstore" || vertical === "travel") {
    ...
    if (fd.accessType === "right_in_out") { locAdj *= 0.88; locNotes.push("right-in/right-out access (-12%)"); }
    if (fd.padSite && fd.padSite.toLowerCase().includes("mid-block")) { locAdj *= 0.85; locNotes.push("mid-block (-15%)"); }
    if (fd.hours === "standard" || fd.hours === "limited") { locAdj *= 0.80; locNotes.push("limited hours (-20%)"); }
    const mhi = parseInt(fd.medianHouseholdIncome) || parseInt(fd.mhi) || 0;
    let mhiAdj = 1.0;
    if (mhi > 0 && mhi < 45000) { mhiAdj = 0.90; locNotes.push("lower-income market (-10%)"); }
    else if (mhi >= 45000 && mhi < 55000) { mhiAdj = 0.95; locNotes.push("moderate-income market (-5%)"); }
    const totalAdj = locAdj * mhiAdj;
  ...
  // ── Foodservice note (c-store / travel — ADDITIVE, not subset) ──
  if (vertical === "cstore" || vertical === "travel") {
    // Derive merchandise range from sqft + income tier
    const storeSqft = parseInt(fd.storeSqft) || parseInt(fd.buildingSqft) || 2675;
    const NACS_AVG_SF = 2675;
    const mhi = parseInt(fd.medianHouseholdIncome) || parseInt(fd.mhi) || 0;
    // sqft scalar: linear scale vs NACS avg (clamped 0.6-1.4)
    const sqftScalar = Math.max(0.6, Math.min(1.4, storeSqft / NACS_AVG_SF));
    // income scalar: $75K+ = 1.0, tighter brackets for lower-income markets
    let incomeScalar = 1.0;
    if (mhi > 0) {
      if (mhi < 40000) incomeScalar = 0.72;
      else if (mhi < 50000) incomeScalar = 0.80;
      else if (mhi < 55000) incomeScalar = 0.85;
      else if (mhi < 65000) incomeScalar = 0.90;
      else if (mhi < 75000) incomeScalar = 0.95;
      // $75K+ stays 1.0
    }
    // Base: $150K "typical new store" total in-store (NACS avg $170K includes mature high-traffic sites)

    // Income-adjusted foodservice ceilings (lower-income = lower ticket spend)
    let fsCeilLow, fsCeilHigh, fsLabel;
    if (fd.hasFullKitchen && fd.hasBrandedFastFood && fd.brandedFastFoodName) {
      fsCeilLow = Math.round(37000 * incomeScalar);
      fsCeilHigh = Math.round(51000 * incomeScalar);
      fsLabel = "branded";
    } else if (fd.hasFullKitchen) {
      fsCeilLow = Math.round(20000 * incomeScalar);
      fsCeilHigh = Math.round(32000 * Math.max(incomeScalar, 0.85)); // hard ceiling $32K for unbranded (was $41K)
      if (incomeScalar >= 0.96) fsCeilHigh = Math.round(36000 * incomeScalar); // higher-income markets can reach $36K
      fsLabel = "unbranded";
    } else if (fd.hasHotExpress) {
      fsCeilLow = 7000; fsCeilHigh = 14000;
      fsLabel = "hotexpress";
    } else if (fd.hasRollerGrill) {
      fsCeilLow = 3000; fsCeilHigh = 7000;
      fsLabel = "rollergrill";
    } else {
      fsCeilLow = 0; fsCeilHigh = 0;
      fsLabel = "none";
    }
    ...
    } else if (fsLabel === "hotexpress") {
      lines.push(`FOODSERVICE NOTE: Site has hot express / grab-and-go food but NO full kitchen. FOODSERVICE is a SEPARATE additive line item. Projected foodservice revenue: $${fsCeilLow.toLocaleString()}-${fsCeilHigh.toLocaleString()}/mo. This is NOT a kitchen — it is pre-packaged hot food. Avg hot express ticket: $5-8 per visit.${mhiNote} MERCHANDISE is projected SEPARATELY at $${merchFloor.toLocaleString()}-${merchCeiling.toLocaleString()}/mo.${sqftNote}`);
    } else if (fsLabel === "rollergrill") {
      lines.push(`FOODSERVICE NOTE: Site has ROLLER GRILL ONLY — NO kitchen, NO hot express. FOODSERVICE is a SEPARATE additive line item. Projected foodservice revenue: $${fsCeilLow.toLocaleString()}-${fsCeilHigh.toLocaleString()}/mo. A roller grill is NOT a kitchen. Avg roller grill ticket: $4-7 per visit.${mhiNote} MERCHANDISE is projected SEPARATELY at $${merchFloor.toLocaleString()}-${merchCeiling.toLocaleString()}/mo.${sqftNote}`);
    } else {
...
const VERT_BENCHMARKS = {
  cstore: {
    ...
    revenueCenters: ["Motor Fuel — Gasoline (gal/mo)", "Motor Fuel — Diesel (gal/mo, if applicable)", "In-Store Merchandise ($/mo)", "Foodservice ($/mo, if applicable)", "Attached QSR ($/mo, additive, if present)"],
    conventions: "NO lottery in c-store sales. Dispensed beverages are part of Merchandise, NOT Foodservice. Foodservice = prepared food only. ALL REVENUE CENTERS ARE ADDITIVE — Merchandise + Foodservice = total in-store sales. Do NOT double-count. NACS avg total in-store (Merchandise + Foodservice combined): ~$170K/mo. TRANSACTION VALUE RULE: Revenue = transactions × avg ticket. Avg in-store basket is $8-12 (NACS). FOODSERVICE RANGES BY KITCHEN TYPE (absolute $/mo): No kitchen/roller grill $3,000-7,000/mo, Hot express $7,000-14,000/mo, Unbranded kitchen $20,000-41,000/mo, Branded kitchen $37,000-51,000/mo. MERCHANDISE is EVERYTHING ELSE. Year 1 kitchen ramp: 60-75% of stabilized. C-STORE DIESEL GUARDRAILS (CRITICAL): Auto diesel only is NOT truck diesel infrastructure. Project auto-only diesel at 3-8% of total fuel volume. HI-FLOW TRUCK LANES exception applies when explicitly present.",
    categoryMix: "MERCHANDISE category mix (% of Merchandise line): Tobacco 18-24%, Packaged bev 15-19%, Beer/wine 10-16%, Dispensed bev 5-8%, Snacks 12-15%, Other 12-20%. FOODSERVICE is a SEPARATE line item.",
  },
  travel: {
    label: "Travel Center",
    ...
    revenueCenters: ["Motor Fuel — Gasoline (gal/mo)", "Motor Fuel — Diesel (gal/mo)", "In-Store Merchandise ($/mo)", "Foodservice ($/mo, if applicable)", "Attached QSR ($/mo, if present)", "Truck Services (parking, showers, scales — $/mo)"],
    conventions: "Same as c-store plus: Diesel is a primary revenue center, not secondary. ALWAYS use the per-lane throughput by brand tier from benchmarks. ALL REVENUE CENTERS ARE ADDITIVE — Merchandise + Foodservice = total in-store. Same foodservice ranges apply by kitchen type. TRANSACTION VALUE RULE: Same as c-store — avg in-store basket $8-12. Trucker basket may reach $10-15. Attached QSR uses brand-specific tickets. NEVER inflate QSR ticket to match a revenue target — adjust transaction count instead.",
    categoryMix: "Similar to c-store but with higher foodservice share (truck drivers eat) and truck-oriented merchandise. Same additive structure — Merchandise and Foodservice are separate line items.",
  },
  qsr: {
...
CRITICAL: The intake form describes the PROPOSED concept — it may or may not match any existing business at that address. Do NOT describe or reference the features, format, or operations of any existing business at the site address. Your research is about the MARKET (demographics, traffic, competition, anchors), NOT about what currently exists at the address. If you find an existing business at the address, note it under Competition if relevant, but do NOT let its features override the intake form.
...
- INTAKE IS SOURCE OF TRUTH FOR SITE FEATURES (CRITICAL): The intake form defines what this site HAS — fuel positions, kitchen type, diesel configuration, square footage, equipment, services. If the intake says "roller grill only" and research mentions a kitchen at the address, USE THE INTAKE. Research describes the MARKET (demographics, traffic, competitors, employers). The intake describes the SITE. NEVER override intake features with research findings. NEVER add features not present in the intake (kitchens, beer caves, additional fuel positions, truck lanes, etc.).
...
NARRATIVE RULES:
- INTAKE IS SOURCE OF TRUTH FOR SITE FEATURES: When describing the site, use ONLY the features from the intake form (fuel positions, kitchen type, square footage, equipment, services). NEVER describe features found in market research that are not in the intake. If the intake says "roller grill only," the report must say "roller grill" — NEVER "full kitchen" even if research mentions one at the address.
...
function clampProjections(analysis, fd, vertical) {
  ...
  if (vertical === "cstore" || vertical === "travel") {
    ...
    if (fd.hours === "standard" || fd.hours === "limited") locAdj *= 0.80;
    const mhi = parseInt(fd.medianHouseholdIncome) || parseInt(fd.mhi) || 0;
    let mhiAdj = 1.0;
    if (mhi > 0 && mhi < 45000) mhiAdj = 0.90;
    else if (mhi >= 45000 && mhi < 55000) mhiAdj = 0.95;
    const totalAdj = locAdj * mhiAdj;
    const gasFloor = Math.round(scaledBase * totalAdj * 0.75);
    ...
    if (gasOnlyKey && p[gasOnlyKey]) {
      if (p[gasOnlyKey].high > gasCeil) {
        ...
        p[gasOnlyKey].low = Math.min(p[gasOnlyKey].low, Math.round(p[gasOnlyKey].mid * 0.75));
      }
      if (p[gasOnlyKey].low < gasFloor) {
        p[gasOnlyKey].low = gasFloor;
        p[gasOnlyKey].mid = Math.max(p[gasOnlyKey].mid, Math.round(gasFloor * 1.15));
        p[gasOnlyKey].high = Math.max(p[gasOnlyKey].high, Math.round(gasFloor * 1.35));
        if (p[gasOnlyKey].high > gasCeil) p[gasOnlyKey].high = gasCeil;
        if (p[gasOnlyKey].mid > p[gasOnlyKey].high) p[gasOnlyKey].mid = Math.round(p[gasOnlyKey].high * 0.88);
      }
    }
  }
  ...
  if (vertical === "cstore" || vertical === "travel") {
    ...
    if (merchKey && p[merchKey]) {
      if (p[merchKey].high > merchCeil) {
        p[merchKey].high = merchCeil;
        p[merchKey].mid = Math.min(p[merchKey].mid, Math.round(merchCeil * 0.85));
        p[merchKey].low = Math.min(p[merchKey].low, Math.round(p[merchKey].mid * 0.75));
        if (p[merchKey].low >= p[merchKey].mid) p[merchKey].low = Math.round(p[merchKey].mid * 0.75);
      }
    }
  }
